...
// Make sure $randomAddress exists and has data
if (!isset($randomAddress) || empty($randomAddress)) {
    die(json_encode(['Response' => 'Failed to generate address']));
}

$num_us = isset($randomAddress['numd']) ? $randomAddress['numd'] : '';
$address_us = isset($randomAddress['address1']) ? $randomAddress['address1'] : '';
$address = $num_us . ' ' . $address_us;
$city_us = isset($randomAddress['city']) ? $randomAddress['city'] : '';
$state_us = isset($randomAddress['state']) ? $randomAddress['state'] : '';
$zip_us = isset($randomAddress['zip']) ? $randomAddress['zip'] : '';

require_once 'genphone.php';
// Make sure $areaCodes exists and has data
if (!isset($areaCodes) || empty($areaCodes)) {
    die(json_encode(['Response' => 'Failed to generate phone']));
}
...
// Get credit card data safely
$cc1 = $_GET['cc'] ?? '';
if (empty($cc1)) {
    die(json_encode(['Response' => 'No credit card data provided']));
}
...
if (empty($cc) || empty($month) || empty($year) || empty($cvv)) {
    die(json_encode(['Response' => 'Incomplete credit card data']));
}
...
// Check if geocoding data is valid
if (!$geocoding_data || !isset($geocoding_data[0])) {
    // Use fallback coordinates or default values
...
$email = "zodmadarabgmiyt@gmail.com"; // Override email
...
if (!$site1) {
    die(json_encode(['Response' => 'No site URL provided']));
}
